...
def _get_player_score(player, round_number):
	...
	for row in range(grid_size):
		...
		if player.lines_number[row] == row + 1:
			...
			if number_of[tc] == 3: future_score += player.SET_BONUS
			...
			if round_number < 5:
				future_score += sum(grid_state[row][c] for c in range(grid_size))
			future_score += sum(grid_state[r][col] for r in range(grid_size))
...
def sort_move(moves, player):
	...
	for move in moves:
		...
		if round_number < 5:
			future_score += addition_floor_penalty[
				min(len(player.floor) - 1, penalty_index + move[2].num_to_floor_line)]
		if row != -1:
			tc = move[2].tile_type
			col = int(player.grid_scheme[row][tc])
			if num_to_pattern_line + player.lines_number[row] == row + 1:
				...
				if (round_number >= 4): score += 1 + above + below + left + right + future_score * FUTURE / 2
				else: score += 1 + above + below + left + right + future_score * FUTURE
				...
				if round_number < 5:
					future_score += sum(grid_state[row][c] for c in range(grid_size))
				future_score += sum(grid_state[r][col] for r in range(grid_size))
				if (round_number >= 4): score += (4 - row + num_to_pattern_line + player.lines_number[row]) / 5 + future_score * FUTURE / 2
				else: score += (4 - row + num_to_pattern_line + player.lines_number[row]) / 5 + future_score * FUTURE
...
def _max_node_value(graph, state, depth, depth_limit, player, alpha, beta):
	...
	for move in moves:
		...
		if round_end or depth == depth_limit:
			...
			succ_state, state_alpha = _min_node_value(graph, next_state, depth, depth_limit, abs(1 - player), alpha,
			                                          beta)
...
def _min_node_value(graph, state, depth, depth_limit, player, alpha, beta):
	...
	for move in moves:
		next_state, round_end = state.next_state(player, move)
		if round_end or depth == depth_limit:
			state_beta = state_eval(next_state, abs(1 - player))
		else:
			succ_state, state_beta = _max_node_value(graph, next_state, depth, depth_limit, abs(1 - player), alpha,
			                                         beta)
